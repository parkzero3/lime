...
<body>
	...
	<form action="./freeBoardInsertPro.ino">
		...
		<c:if test="${user ne null and user.name == freeBoardDto.name }"><input type="button" value="수정" onclick="edit_confirm()" /></c:if>
		<c:if test="${user ne null and user.name == freeBoardDto.name }"><input type="button" value="삭제" onclick="delete_confirm()" /></c:if>
