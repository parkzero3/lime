...
<script>
	...
	window.onload = function(){
        ...
        $('#search').click(function(){
        	...
        	console.log("search 접속");
        	var sel = document.getElementById("searchKey");
    		var searchKey = sel.options[sel.selectedIndex].value;
    		var searchWord = document.getElementById("searchWord").value;
    		var data = document.getElementById("dataCnt");
    		var dataCnt = data.options[data.selectedIndex].value;
        	
    		var url = 'http://localhost:9090<%=request.getContextPath()%>/freeBoardListAjax.ino';
        	
    		$.ajax({
    			
    			type : 'GET',
    			data : {searchKey:searchKey,searchWord:searchWord,dataCnt:dataCnt,now:'${now}'},
    			url : url,
    			dataType : "json",
    			...
    			success : function(data){
    				console.log("success");
    				if( data.freeBoardList != null){
    					cosole.log("list"+data.freeBoardList);
    				}else{
    					alert("데이터전송실패");
    				}
    			},error:function(data){
...
<body>
	...
	<div>
		<select name="dataCnt" id="dataCnt">
